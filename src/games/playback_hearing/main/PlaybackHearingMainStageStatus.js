...
import {PlaybackHearingMainCallback} from './PlaybackHearingMainCallback';
import {MtcDiceRoller} from '../../../dice';
import {PlaybackHearingGameResult} from '../PlaybackHearingGameResult';
import {MtcDiceFace} from "../../../dice/MtcDiceFace";

const PHASES = Symbol();

export class PlaybackHearingMainStageStatus extends StageStatus {
  static get PHASES() {
    ...
    return PlaybackHearingMainStageStatus[PHASES];
  }

  constructor(gameStatus, callbacks) {
    super(gameStatus);

    check.assert.instance(callbacks, PlaybackHearingMainCallback);
  ...
  _startCountdown() {
    this._countdownTimer = setTimeout(
      () => this._callbacks.onGameFinished(new PlaybackHearingGameResult(this.areAllGuessed(), this.guessed, this.failed)),
