...
import {AttentionalSpanMainStageStatus} from './AttentionalSpanMainStageStatus';
import {backgroundTiledImage, cancelImage, confirmImage, frameImage, unrecognized} from '../../../assets';
...
export class AttentionalSpanMainRenderer extends StageRenderer {
  ...
  create() {
    ...
    this.checkButton = new GameButton(this.worldHorizontalMiddle, this.worldHeight - 40, 200, 40, this.getText('game.attentionalSpan.checkBtn'), this.checkResponse, this, null, null);
  ...
  update() {
    ...
    if (this.status.phase === AttentionalSpanMainStageStatus.PHASES.DICE_SHOWING && this.status.currentDiceIteration < this.game.configuration.parameterValues.numberOfElements) {
      this.diceShowingPhaseUpdate();
    }
  }

  diceShowingPhaseUpdate() {
    if (!this.status.isShowingDice) {
      ...
      if (Date.now() > this.diceShowTimestamp + (this.game.configuration.parameterValues.timeBetweenElements * 1000)) {
        ...
        if (this.status.currentDiceIteration === this.game.configuration.parameterValues.numberOfElements) {
          if (this.game.configuration.responseIntroduction === AttentionalSpanGameMetadata.RESPONSE_TYPES[1]) {
            this.shownSprites.reverse();
          }
          this.status.phase = AttentionalSpanMainStageStatus.PHASES.DICE_WRITING;
  ...
  checkResponse() {
    ...
    this.shownSprites.forEach((value, index) => {
  ...
  updateTime() {
    ...
    if (this.status.phase === AttentionalSpanMainStageStatus.PHASES.DICE_WRITING) {
