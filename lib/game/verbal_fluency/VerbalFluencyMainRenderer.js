...
export default class VerbalFluencyMainRenderer extends GameRenderer {
    ...
    create() {
        ...
        for (let i = 0, len = MtcDiceFace.LETTERS_FACE_VALUES.length; i < len; i++) {
            ...
            currentSprite.scale.setTo(this.game.status.diceScale, this.game.status.diceScale);
            currentSprite.anchor.setTo(0.5, 0.5);
    ...
    realWord() {
        return this.game.status.currentWord.join("").replace(/\s+/g, '');
    }
    ...
    onWordCheck() {
        let checkResult = {
            timeLeft: this.game.status.config.time - this.stateElapsedTime(),
            repeated: false,
            validWord: false
        }

        if (this.game.status.possibleWords.indexOf(this.realWord().toLowerCase()) !== -1) {
            if (this.game.status.correctWords.indexOf(this.realWord().toLowerCase()) !== -1) {
                checkResult.repeated = true;
                this.repeatedWords++;
            } else {
                this.game.status.correctWords.push(this.realWord().toLowerCase());
                checkResult.validWord = true;
            }
        }

        this.game.status.guessAttempts++;

        if (this.game.status.gameMode == GameStatus.GAME_MODES.NORMAL) {
            this.game.status.config.gameCallback.wordChecked(checkResult);
        }
    ...
    onWordReset(callback = true) {
        if (callback && this.game.status.gameMode == GameStatus.GAME_MODES.NORMAL) {
            this.game.status.config.gameCallback.wordReset();
        }

        this.game.status.currentWord = new Array(' ', ' ', ' ', ' ', ' ', ' ', ' ');
        let randomCoords = Array(0, 0);
        for (let i = 0, len = MtcDiceFace.LETTERS_FACE_VALUES.length; i < len; i++) {
            do {
                randomCoords = this.getRandomScrambleCoords();
    ...
    onDragStart(sprite, pointer) {
        ...
        let animation = this.game.add.tween(sprite.scale).to(
            { x: this.game.status.diceScale_DRAGGED, y: this.game.status.diceScale_DRAGGED },
            50,
            Phaser.Easing.Linear.None,
            true,
            0,
    ...
    onDragStop(sprite, pointer) {
        ...
        if (currentLetterIndex >= 0) {
            this.game.status.currentWord[currentLetterIndex] = ' ';
        }

        //If it was bound to something insert the letter into the temporal word
        if (boundToSomething && boundIndex != -1) {
            this.game.status.currentWord[boundIndex] = currentLetter;
        }

        let animation = this.game.add.tween(sprite.scale).to(
            { x: this.game.status.diceScale, y: this.game.status.diceScale },
            50,
            Phaser.Easing.Linear.None,
            true,
            0,
    ...
    onGameEndCallback() {
        let info = {
            totalAttempts: this.game.status.guessAttempts,
            totalGuessedWords: this.game.status.correctWords.length,
            guessedWords: this.game.status.correctWords,
            repeatedGuesses: this.game.status.repeatedWords
        }
