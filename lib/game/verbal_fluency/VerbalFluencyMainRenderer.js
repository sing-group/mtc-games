...
import VerbalFluencyStatus from './VerbalFluencyStatus';
...
export default class VerbalFluencyMainRenderer extends GameRenderer {
    ...
    update() {
        ...
        const remainingTime = this.game.status.config.time - this.stateElapsedTime();
    ...
    isLetterColliding(x, y) {
        ...
        tempSprite.scale.setTo(this.game.status.config.diceScale, this.game.status.config.diceScale);
        ...
        return this.letterSprites.some(element => this.getDistance(x, y, element.finalX, element.finalY) < minimumSeparation);
    }
    ...
    onDragStop(sprite, pointer) {
        ...
        const currentLetterIndex = this.game.status.currentWord.indexOf(currentLetter);
        ...
        const finalScale = boundToSomething ? { x: this.game.status.diceScaleDocked, y: this.game.status.diceScaleDocked } : { x: this.game.status.config.diceScale, y: this.game.status.config.diceScale };
