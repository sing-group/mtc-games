...
export default class VerbalFluencyMainRenderer extends GameState {
    ...
    preload() {
        ...
        switch (this.game.gameConfig.locale) {
            ...
            case "en_US":
                ...
                break;
            case "gl_ES":
              this.possible_words = VerbalFluencyDictionary.gl_ES;
                break;
            default:
                this.possible_words = VerbalFluencyDictionary.en_US;
        }
    ...
    update() {
        ...
        this.game.timeText.setText(
            remainingTime > 0 ? this.game.i18n.text("game.standard.time") + ": " + Math.ceil(this.game.gameConfig.time - this.stateElapsedTime()) : this.game.i18n.text("game.standard.timeIsUp")
        );
        this.updateScore();
    ...
    onWordCheck() {
        ...
        if (this.game.gameMode == Game.GAME_MODES.NORMAL) {
            this.game.gameConfig.gameCallback.wordChecked(checkResult);
        }
    ...
    onWordReset(callback = true) {
        if (callback && this.game.gameMode == Game.GAME_MODES.NORMAL) {
            this.game.gameConfig.gameCallback.wordReset();
        }
