...
export default class RecognitionGameMainRenderer extends StageRenderer {
    ...
    update() {
        ...
        if (this.game.status.phase === RecognitionStatus.PHASES.DICE_SHOW && this.game.status.currentDiceIteration < this.game.configuration.numberOfElements) {
    ...
    diceShowPhaseUpdate() {
        if (!this.game.status.isShowingDice) {
            ...
            this.currentShowSprite.scale.setTo(this.configuration.diceScaleShown, this.configuration.diceScaleShown);
            ...
            if (Date.now() > this.dice_show_timestamp + (this.game.configuration.timePerElement * 1000)) {
                ...
                if (this.game.status.currentDiceIteration === this.game.configuration.numberOfElements) {
    ...
    diceSelectPhaseUpdate() {
        ...
        if (!this.game.status.dicesToSelectShown) {
            ...
            for (let i = 0, len = this.game.status.stimulus_values.length; i < len; i++) {
                ...
                currentSprite.scale.setTo(this.configuration.diceScale, this.configuration.diceScale);
