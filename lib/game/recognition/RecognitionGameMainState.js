...
export default class RecognitionGameMainState extends GameState {
    ...
    update() {
        ...
        this.game.timeText.setText(
            remainingTime > 0 ? "Time: " + Math.ceil(remainingTime) : "Time's up"
        );
        this.updateScore();
    ...
    diceShowPhaseUpdate() {
        // Don't run if game is already ended
        if (!this.gameRunning) {
            return;
        }

        if (!this.isShowingDice) {
            console.log('not showing any dices. show one now');
            ...
            if (Date.now() > this.dice_show_timestamp + (this.game.gameConfig.timePerElement * 1000)) {
                console.log('moment to hide the dice!');
