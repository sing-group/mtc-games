...
export default class RecognitionGameStartRenderer extends GameRenderer {
    ...
    preload() {
        ...
        this.game.status = new RecognitionStatus(this.game.status);
    ...
    create() {
        ...
        this.game.gameDescriptionText = this.game.add.text(
            ...
            this.game.i18n.text(I18NId.forGame("recognition").param("timePerElement").name()) + ": " + this.game.status.config.timePerElement + "\n" +
            this.game.i18n.text(I18NId.forGame("recognition").param("numberOfElements").name()) + ": " + this.game.status.config.numberOfElements,
            style
        );
        this.game.gameDescriptionText.anchor.setTo(0.5, 0.5);
    ...
    onStartGame() {
        this.game.status.gameMode = GameStatus.GAME_MODES.NORMAL;
        this.game.status.try++;
        this.game.state.start("MainRenderer", true, true);
    }

    onStartTutorial() {
        this.game.status.gameMode = GameStatus.GAME_MODES.TUTORIAL;
        this.game.state.start("MainRenderer", true, true);
    }
}